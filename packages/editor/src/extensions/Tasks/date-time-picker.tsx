...
export function DateTimePicker({ preset, bigger }: { preset?: Task['due']; bigger?: boolean }) {
      ...
      <PopoverTrigger asChild>
        ...
        >
          ...
          {date?.from ? (
            ...
            <>
              {!bigger && <CalendarDays className={cn('h-4 w-4 text-foreground', bigger && 'h-4 w-4')} />}
              <span>{bigger ? 'Empty' : 'Add Due'}</span>
            </>
          )}
        </Button>
      </PopoverTrigger>
      <PopoverContent align={bigger ? 'center' : 'start'} className="w-auto p-0">
        <Calendar mode="range" numberOfMonths={1} selected={date} onSelect={setDate} initialFocus />
          ...
          <div className="flex items-center justify-between gap-1 ">
            ...
            <Switch
              ...
              onCheckedChange={c => {
                ...
                if (!c) {
                  setDate({
                    from: date?.from ? new Date(date.from.setHours(0, 0, 0, 0)) : undefined,
                    to: date?.to ? new Date(date.to.setHours(0, 0, 0, 0)) : undefined,
                  })
                }
              }}
            />
